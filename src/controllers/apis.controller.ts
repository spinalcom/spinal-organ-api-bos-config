...
export class APIController extends Controller {
	...
	public async getAllBosApiRoute(@Request() req: any): Promise<IApiRoute[] | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, [ADMIN_APPS.api_routes.name, ADMIN_APPS.user_profiles.name, ADMIN_APPS.app_profiles.name]);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async deleteBosApiRoute(@Request() req: any, @Path() id: string): Promise<{ message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.api_routes.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
