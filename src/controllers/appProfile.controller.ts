...
import { getProfileId, isAdminOrHasAccessToAdminApp } from "../security/authentication";
...
export class AppProfileController extends Controller {
	...
	public async createAppProfile(@Request() req: express.Request, @Body() data: IProfile): Promise<IProfileData | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.app_profiles.name);
	...
	public async profileHasAccessToContext(@Request() req: express.Request, @Path() profileId: string, @Path() contextId: string, @Query() digitalTwinId?: string) {
		try {
			...
			const hasAccessToContext = await serviceInstance.profileHasAccessToContext(profileId, contextId, digitalTwinId);
			this.setStatus(HTTP_CODES.OK);
			return hasAccessToContext ? true : false;
		} catch (error: Error | any) {
			this.setStatus(error.code || HTTP_CODES.INTERNAL_ERROR);
			return { message: error.message };
		}
	...
	public async profileHasAccessToApi(@Request() req: express.Request, @Path() profileId: string, @Path() apiId: string) {
		try {
			...
			const hasAccessToApi = await serviceInstance.profileHasAccessToApi(profileId, apiId);
			this.setStatus(HTTP_CODES.OK);
			return hasAccessToApi ? true : false;
		} catch (error: Error | any) {
			this.setStatus(error.code || HTTP_CODES.INTERNAL_ERROR);
			return { message: error.message };
		}
