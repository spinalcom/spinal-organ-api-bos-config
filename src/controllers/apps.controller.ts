...
import { AppService, AppsType, UserListService, UserProfileService } from "../services";
import { Body, Controller, Delete, Get, Path, Post, Put, Route, Security, Tags, UploadedFile, Request } from "tsoa";
import { BUILDING_SUB_APP_TYPE, HTTP_CODES, SECURITY_MESSAGES, SECURITY_NAME, SUB_APP_RELATION_NAME } from "../constant";
import { ISpinalApp } from "../interfaces";
import * as express from "express";
import { AuthError } from "../security/AuthError";
import { checkAndGetTokenInfo, getProfileId, isAdminOrHasAccessToAdminApp } from "../security/authentication";
...
export class AppsController extends Controller {
	...
	public async createAdminApp(@Request() req: express.Request, @Body() appInfo: ISpinalApp): Promise<ISpinalApp | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
	...
	public async createBuildingApp(@Request() req: express.Request, @Body() appInfo: ISpinalApp): Promise<ISpinalApp | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async createBuildingSubApp(@Request() req: express.Request, @Path() appId: string, @Body() appInfo: ISubApp): Promise<ISubApp | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
			const profileId = await getProfileId(req);
			const appNode = await UserProfileService.getInstance().profileHasAccessToApp(searchById, profileId, appId);
	...
	public async getAllAdminApps(@Request() req: express.Request): Promise<ISpinalApp[] | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, [ADMIN_APPS.apps.name, ADMIN_APPS.user_profiles.name, ADMIN_APPS.app_profiles.name]);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async getAllBuildingApps(@Request() req: express.Request): Promise<ISpinalApp[] | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, [ADMIN_APPS.apps.name, ADMIN_APPS.user_profiles.name, ADMIN_APPS.app_profiles.name]);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	@Security(SECURITY_NAME.bearerAuth)
	@Get("/get_all_apps")
	public async getAllApps(@Request() req: express.Request): Promise<{ apps: ISpinalApp[]; adminApps: ISpinalApp[] } | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, [ADMIN_APPS.apps.name, ADMIN_APPS.user_profiles.name, ADMIN_APPS.app_profiles.name]);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async getAdminApp(@Request() req: express.Request, @Path() appNameOrId: string): Promise<ISpinalApp | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, [ADMIN_APPS.apps.name, ADMIN_APPS.user_profiles.name, ADMIN_APPS.app_profiles.name]);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);

			const node = await appServiceInstance.getAdminApp(searchByNameOrId, appNameOrId);
			if (node) {
				this.setStatus(HTTP_CODES.OK);
				return node.info.get();
			}
	...
	public async getBuildingApp(@Request() req: express.Request, @Path() appNaneOrId: string): Promise<ISpinalApp | { message: string }> {
		try {
			const profileId = await getProfileId(req);
			const node = await UserProfileService.getInstance().profileHasAccessToApp(searchByNameOrId, profileId, appNaneOrId);
	...
	public async getBuildingSubApp(@Request() req: express.Request, @Path() appNameOrId: string, @Path() subAppNameOrId: string): Promise<any | { message: string }> {
		try {
			const profileId = await getProfileId(req);
			const node = await UserProfileService.getInstance().profileHasAccessToSubApp(searchByNameOrId, profileId, appNameOrId, subAppNameOrId);
	...
	public async updateAdminApp(@Request() req: express.Request, @Path() appId: string, @Body() newInfo: ISpinalApp): Promise<ISpinalApp | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async updateBuildingApp(@Request() req: express.Request, @Path() appId: string, @Body() newInfo: ISpinalApp): Promise<ISpinalApp | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async updateBuildingSubApp(@Request() req: express.Request, @Path() appId: string, @Path() subAppId: string, @Body() newInfo: ISubApp): Promise<ISubApp | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async deleteBuildingSubApp(@Request() req: express.Request, @Path() appId: string, @Path() subAppId: string): Promise<{ message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async uploadAdminApp(@Request() req: express.Request, @UploadedFile() file): Promise<ISpinalApp[] | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
			...
			const apps = await appServiceInstance.uploadApps(AppsType.admin as "admin", file.buffer, isExcel);
	...
	public async uploadBuildingApp(@Request() req: express.Request, @UploadedFile() file): Promise<ISpinalApp[] | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
			...
			const apps = await appServiceInstance.uploadApps(AppsType.building as "building", file.buffer, isExcel);
	...
	public async uploadBuildingSubApp(@Request() req: express.Request, @UploadedFile() file): Promise<{ subApps?: ISpinalApp[]; errors?: string[] | string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.apps.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async getFavoriteApps(@Request() request: express.Request) {
		try {
			...
			const subApps = nodes.filter((node) => node.info.type.get() === BUILDING_SUB_APP_TYPE);
			const parents = await Promise.all(subApps.map((node) => node.getParents(SUB_APP_RELATION_NAME)));
			...
			const res = await AppService.getInstance().formatAppsAndAddSubApps(Array.from(apps), subApps);
			this.setStatus(HTTP_CODES.OK);
			return res;
		} catch (error: Error | any) {
			this.setStatus(error.code || HTTP_CODES.INTERNAL_ERROR);
	...
	public async addAppToFavoris(@Request() request: express.Request, @Body() data: { appIds: string[] }) {
		try {
			...
			await UserListService.getInstance().addFavoriteApp(userName, profileId, data.appIds);
			const nodes = await UserListService.getInstance().getFavoriteApps(userName);
	...
	public async removeAppFromFavoris(@Request() request: express.Request, @Body() data: { appIds: string[] }) {
		try {
			...
			await UserListService.getInstance().removeFavoriteApp(userName, profileId, data.appIds);
			const nodes = await UserListService.getInstance().getFavoriteApps(userName);
