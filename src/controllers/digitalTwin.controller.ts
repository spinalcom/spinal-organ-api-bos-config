...
import { isAdminOrHasAccessToAdminApp } from "../security/authentication";
...
export class DigitaltwinController extends Controller {
	...
	public async addDigitalTwin(@Request() req: express.Request, @Body() data: { name: string; url: string }, @Query() set_as_actual_digitaltwin?: boolean) {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.digital_twin.name);
	...
	public async getDefaultDigitalTwinContexts(@Request() req: express.Request) {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, [ADMIN_APPS.digital_twin.name, ADMIN_APPS.user_profiles.name, ADMIN_APPS.app_profiles.name]);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
