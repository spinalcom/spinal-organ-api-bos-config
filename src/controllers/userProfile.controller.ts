...
import { checkIfItIsAdmin, getProfileId, isAdminOrHasAccessToAdminApp } from "../security/authentication";
...
export class UserProfileController extends Controller {
	...
	public async createUserProfile(@Request() req: express.Request, @Body() data: IProfile): Promise<IProfileData | { message: string }> {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.user_profiles.name);
	...
	public async profileHasAccessToApp(@Request() req: express.Request, @Path() profileId: string, @Path() appId: string) {
		try {
			...
			const profileHasAccessToApp = await serviceInstance.profileHasAccessToApp(searchById, profileId, appId);
			this.setStatus(HTTP_CODES.OK);
	...
	@Security(SECURITY_NAME.bearerAuth)
	@Get("/profile_has_access_to_sub_app/{profileId}/{appNameOrId}/{subAppNameOrId}")
	public async profileHasAccessToSubApp(@Request() req: express.Request, @Path() profileId: string, @Path() appNameOrId: string, @Path() subAppNameOrId: string) {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.user_profiles.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);

			const hasAccessToSubApp = await serviceInstance.profileHasAccessToSubApp(searchById, profileId, appNameOrId, subAppNameOrId);
			this.setStatus(HTTP_CODES.OK);
