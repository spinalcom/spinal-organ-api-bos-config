...
export class WebsocketLogsController extends Controller {
	private _websocketLogService: WebsocketLogsService = WebsocketLogsService.getInstance();
	...
	public async getWebsocketState(@Request() req: express.Request) {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.websocket_state.name);
	...
	public async readWebsocketLogs(@Request() req: express.Request, @Path() begin: string | number, @Path() end: string | number) {
		try {
			const hasAccess = await isAdminOrHasAccessToAdminApp(req, ADMIN_APPS.websocket_state.name);
			if (!hasAccess) throw new AuthError(SECURITY_MESSAGES.UNAUTHORIZED);
	...
	public async readCurrentWeekLogs(@Request() req: express.Request) {
		try {
			...
			const { end, start } = SpinalServiceLog.getInstance().getDateFromLastDays(7);
	...
	public async readCurrentYearLogs(@Request() req: express.Request) {
		try {
			...
			const { end, start } = SpinalServiceLog.getInstance().getDateFromLastDays(365);
			return this._websocketLogService.getFromIntervalTime(start, end);
		} catch (error: Error | any) {
			this.setStatus(error.code || HTTP_CODES.INTERNAL_ERROR);
			return { message: error.message };
