...
export class AdminProfileService {
  ...
  async addDigitalTwinToAdminProfile(
    digitalTwins: SpinalNode | SpinalNode[]
  ): Promise<SpinalNode[]> {
    if (!Array.isArray(digitalTwins)) digitalTwins = [digitalTwins];

    return digitalTwins.reduce(
      async (prom: Promise<SpinalNode[]>, digitalTwin: SpinalNode) => {
        const liste = await prom;

        try {
          const node = await this._adminNode.addChild(
            digitalTwin,
            PROFILE_TO_AUTHORIZED_DIGITAL_TWIN_RELATION_NAME,
            PTR_LST_TYPE
          );
