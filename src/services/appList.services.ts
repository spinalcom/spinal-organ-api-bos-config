...
import {
  ...
  TOKEN_TYPE,
  TOKEN_RELATION_NAME,
  APP_LIST_CONTEXT_NAME,
  APP_CATEGORY_TYPE,
  APP_LIST_CONTEXT_TYPE,
  CONTEXT_TO_USER_RELATION_NAME,
  CONTEXT_TO_APP_RELATION_NAME,
} from '../constant';
import {
  IAppCredential,
  IApplicationToken,
...
export class AppListService {
  ...
  public async authenticateApplication(
    application: IAppCredential | IOAuth2Credential
  ): Promise<{code: number; data: string | IApplicationToken}> {
    ...
    return axios
      ...
      .then(async (result) => {
        ...
        data.userInfo = await this._getApplicationInfo(
          data.applicationId,
          adminCredential,
          data.token
        );
  ...
  public _getProfileInfo(userToken: string, adminCredential: IPamCredential) {
