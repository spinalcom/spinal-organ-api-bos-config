...
export class AppService {
	 ...
	 * @memberof AppService
	 */
	public async createAdminApp(appInfo: ISpinalApp): Promise<SpinalNode | undefined> {
	 ...
	 * @memberof AppService
	 */
	public async createBuildingApp(appInfo: ISpinalApp, silenceAlreadyExist = false): Promise<SpinalNode | undefined> {
	 ...
	 */
	public async createOrUpadteAdminApp(appInfo: ISpinalApp): Promise<SpinalNode | undefined> {
		const groupNode = await this._getApplicationGroupNode(ADMIN_APPS_GROUP_NAME, ADMIN_APPS_GROUP_TYPE, true);
		if (!groupNode) return;
	 ...
	 * @memberof AppService
	 */
	public async getBuildingSubApp(searchKeys: TAppSearch, appNameOrId: string, subAppNameOrId: string): Promise<SpinalNode | undefined> {
	 ...
	 */
	public async findBuildingSubAppInApps(searchKeys: TAppSearch, appsNodes: SpinalNode[], subAppNameOrId: string): Promise<SpinalNode | undefined> {
	 ...
	 */
	public async formatAppAndAddSubApps(appsNode: SpinalNode, subAppsNodes?: SpinalNode[]): Promise<ISpinalApp | undefined> {
		const res = appsNode.info.get();
		if (res.type === BUILDING_APP_TYPE) {
			const subApps = await appsNode.getChildren([SUB_APP_RELATION_NAME]);
			if (subApps.length !== 0) {
				res.subApps = subApps.reduce<any[]>((acc, el: SpinalNode) => {
	 ...
	 */
	public async getAllApps(): Promise<{ apps: SpinalNode[]; adminApps: SpinalNode[] }> {
		return {
	 ...
	 * @returns {Promise<SpinalNode>} A promise that resolves to the found SpinalNode or undefined.
	 */
	public async getApps(searchKeys: TAppSearch, appNameOrId: string): Promise<SpinalNode | undefined> {
	 ...
	 */
	public async updateAdminApp(appId: string, newInfo: ISpinalApp): Promise<SpinalNode | undefined> {
	 ...
	 * @throws Will throw an error if the application node cannot be found or the update fails.
	 */
	public async updateBuildingApp(appId: string, newInfo: ISpinalApp): Promise<SpinalNode | undefined> {
	 ...
	 */
	public async updateBuildingSubAppInfo(appId: string, subAppId: string, newInfo: ISubApp): Promise<SpinalNode | undefined> {
		...
		if (subAppNode) {
			...
			if (!subAppNode.element) await subAppNode.getElement();
	...
	private _formatAppsJson(jsonData: ISpinalApp[]): ISpinalApp[] {
		return jsonData.reduce((liste: ISpinalApp[], app: ISpinalApp) => {
