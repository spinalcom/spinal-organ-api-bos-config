...
export default class AuthorizationService {
  ...
  public async authorizeProfileToAccessContext(
    ...
    const node = await this.getAuthorizedDigitalTwinNode(
      profile,
      digitalTwinId,
      true
    );
    ...
    return contextIds.reduce(async (prom, contextId) => {
      ...
      const context =
        await DigitalTwinService.getInstance().findContextInDigitalTwin(
          digitalTwinId,
          contextId
        );
  ...
  public async getAuthorizedContexts(
    ...
    const digitalTwin = await this.getAuthorizedDigitalTwinNode(
      profile,
      digitalTwinId
    );
  ...
  public async getAuthorizedDigitalTwinNode(
    profile: SpinalNode,
    digitalTwinId: string,
    ...
    const digitalTwins = await profile.getChildren(
      PROFILE_TO_AUTHORIZED_DIGITAL_TWIN_RELATION_NAME
    );
    const found = digitalTwins.find(
      (el) =>
        el.getId().get() === digitalTwinId ||
        el.info.refId?.get() === digitalTwinId
    );
    if (found) return found;

    if (createIfNotExist) {
      const digitalTwin = await DigitalTwinService.getInstance().getDigitalTwin(
        digitalTwinId
      );
      if (!digitalTwin) return;
      const ref = await this._createNodeReference(digitalTwin);
      return profile.addChild(
        ref,
        PROFILE_TO_AUTHORIZED_DIGITAL_TWIN_RELATION_NAME,
        PTR_LST_TYPE
      );
