...
import { _getProfileInfo as getAppProfileInfo } from "../utils/ApplicationAuthUtils";
import { _getUserProfileInfo } from "../utils/UserAuthUtils";
...
export class TokenService {
	 ...
	 */
	public async purgeToken(): Promise<(IUserToken | IApplicationToken | undefined)[]> {
	...
	public async getTokenData(token: string): Promise<any> {
		...
		if (data) return data;
		const children = await this.context!.getChildren([TOKEN_RELATION_NAME]);
		const found = children.find((node) => node.getName().get() === token);

		if (!found) return this._checkTokenNearAuthPlateform(token);
	 ...
	 */
	public async tokenIsValid(token: string, deleteIfExpired: boolean = false): Promise<IUserToken | IApplicationToken | undefined> {
	...
	private async _checkTokenNearAuthPlateform(token: string) {
		try {
	...
	private async _checkRequest(authPlateform: any, token: string, actor: "user" | "application") {
		...
		if (actor === "application") info.profile = await getAppProfileInfo(token, authPlateform.urlAdmin, authPlateform);
		else info.profile = await _getUserProfileInfo(token, authPlateform);
