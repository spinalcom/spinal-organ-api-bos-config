...
export class TokenService {
  ...
  public async addTokenToContext(
    token: string,
    data: any
  ): Promise<SpinalNode> {
    const node = new SpinalNode(token, TOKEN_TYPE, new Model(data));
    const child = await this.context.addChildInContext(
      node,
      TOKEN_RELATION_NAME,
      PTR_LST_TYPE
    );
  ...
  private async _checkRequest(
    ...
    const instance =
      actor === 'user'
        ? UserListService.getInstance()
        : AppListService.getInstance();
