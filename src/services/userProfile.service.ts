...
export class UserProfileService {
  ...
  public async createUserProfile(userProfile: IProfile): Promise<IProfileRes> {
    ...
    if (userProfile.appsIds) obj.apps = await this.authorizeProfileToAccessApps(profileNode, userProfile.appsIds);
  ...
  public async updateUserProfile(userProfileId: string, userProfile: IProfileAuthEdit): Promise<IProfileRes> {
    ...
    if (userProfile.appsIds) await this.authorizeProfileToAccessApps(profileNode, userProfile.appsIds);
  ...
  public async getAutorizationStructure(userProfile: string | SpinalNode, digitalTwinId?: string): Promise<IProfileAuthRes> {
    ...
    return {
      ...
      ...(profile.getType().get() === ADMIN_PROFILE_TYPE && { adminApps: await this.getAuthorizedAdminApps(profile) })
    }
  }
