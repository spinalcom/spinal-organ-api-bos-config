...
export class UserProfileService {
	 ...
	 */
	public async getUserProfile(userProfile: string | SpinalNode): Promise<IProfileRes | undefined> {
	 ...
	 */
	public async updateUserProfile(userProfileId: string, userProfile: IProfileAuthEdit): Promise<IProfileRes | undefined> {
		const profileNode = await this._getUserProfileNode(userProfileId);
		if (!profileNode) return;

		if (userProfile.name) this._renameProfile(profileNode, userProfile.name);
	...
	public async getAllUserProfile(): Promise<IProfileRes[]> {
		...
		return Promise.all(promises).then((res) => res.filter((el) => el !== undefined) as IProfileRes[]);
	}
	 ...
	 */
	public getAllUserProfileNodes(): Promise<SpinalNode[]> {
	...
	public async authorizeProfileToAccessContext(userProfile: string | SpinalNode, contextIds: string | string[], digitalTwinId?: string): Promise<SpinalContext[]> {
		...
		if (!profile) return [];
		return authorizationInstance.authorizeProfileToAccessContext(profile, digitalTwinId as string, contextIds);
	}
	 ...
	 */
	public async getAutorizationStructure(userProfile: string | SpinalNode, digitalTwinId?: string): Promise<IProfileAuthRes | undefined> {
		const profile = userProfile instanceof SpinalNode ? userProfile : await this._getUserProfileNode(userProfile);
	...
	public async unauthorizeProfileToAccessContext(userProfile: string | SpinalNode, contextIds: string | string[], digitalTwinId?: string): Promise<SpinalContext[]> {
		const profile = userProfile instanceof SpinalNode ? userProfile : await this._getUserProfileNode(userProfile);
		if (!profile) return [];
		return authorizationInstance.unauthorizeProfileToAccessContext(profile, digitalTwinId as string, contextIds);
	}
	 ...
	 */
	public async profileHasAccessToContext(userProfile: string | SpinalNode, contextId: string, digitalTwinId?: string): Promise<SpinalNode | undefined> {
		const profile = userProfile instanceof SpinalNode ? userProfile : await this._getUserProfileNode(userProfile);
		if (!profile) return undefined;
		return authorizationInstance.profileHasAccessToContext(profile, digitalTwinId as string, contextId);
	}
	 ...
	 */
	public async profileHasAccessToApp(searchKeys: TAppSearch, userProfile: string | SpinalNode, appId: string): Promise<SpinalNode | undefined> {
		const profile = userProfile instanceof SpinalNode ? userProfile : await this._getUserProfileNode(userProfile);
		if (!profile) return undefined;
	 ...
	 */
	public async profileHasAccessToSubApp(searchKeys: TAppSearch, userProfile: string | SpinalNode, appNameOrId: string, subAppNameOrId: string): Promise<SpinalNode | undefined> {
		const profile = userProfile instanceof SpinalNode ? userProfile : await this._getUserProfileNode(userProfile);
		if (!profile) return undefined;
	 ...
	 */
	public async profileHasAccessToApi(userProfile: string | SpinalNode, apiId: string): Promise<SpinalNode | undefined> {
		const profile = userProfile instanceof SpinalNode ? userProfile : await this._getUserProfileNode(userProfile);
		if (!profile) return undefined;
	...
	public async getAuthorizedContexts(userProfile: string | SpinalNode, digitalTwinId?: string): Promise<SpinalContext[]> {
		const profile = userProfile instanceof SpinalNode ? userProfile : await this._getUserProfileNode(userProfile);
		if (!profile) return [];
		return authorizationInstance.getAuthorizedContexts(profile, digitalTwinId as string);
	}
	...
	public async _getUserProfileNode(userProfileId: string): Promise<SpinalNode | undefined> {
