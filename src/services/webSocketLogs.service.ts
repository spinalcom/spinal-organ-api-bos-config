...
import { SpinalServiceLog, ILog, WEBSOCKET_STATE, SpinalLog } from "spinal-service-pubsub-logs";
...
import { WEBSOCKET_LOG_CONTEXT_NAME, WEBSOCKET_LOG_CONTEXT_TYPE } from "../constant";

export default class WebsocketLogsService {
	private static _instance: WebsocketLogsService;
	private _alertTime: number = (process.env.WEBSOCKET_ALERT_TIME && parseInt(process.env.WEBSOCKET_ALERT_TIME)) || 60 * 1000;
	private timeoutIds: { [key: string]: any } = {};
	private _directory: spinal.Directory | undefined;
	private _spinalQueue: SpinalQueue = new SpinalQueue();
	private _logPromMap: Map<string, SpinalLog> = new Map();
	private _lastSendTime: number | undefined;
	private _io: Server | undefined;
	private context: SpinalContext | undefined;
	...
	public async init() {
		this.context = await configServiceInstance.getContext(WEBSOCKET_LOG_CONTEXT_NAME);
		if (!this.context) {
			this.context = await configServiceInstance.addContext(WEBSOCKET_LOG_CONTEXT_NAME, WEBSOCKET_LOG_CONTEXT_TYPE);
		}
	...
	public createLog(type: string, action: string, targetInfo?: { id: string; name: string }, nodeInfo?: { id: string; name: string; [key: string]: string }) {
	...
	public async getLogModel(): Promise<SpinalLog | undefined> {
	...
	public async getDataFromLastHours(numberOfHours: number) {
		const spinalLog = await this.getLogModel();
		if (!spinalLog) return [];
		return SpinalServiceLog.getInstance().getDataFromLastHours(spinalLog, numberOfHours);
	}
	...
	public async getFromIntervalTime(start: string | number | Date, end: string | number | Date) {
		const spinalLog = await this.getLogModel();
		if (!spinalLog) return [];
	...
	private async _addLogs(logType: string, action: string, targetInfo?: { id: string; name: string }, nodeInfo?: { id: string; name: string; [key: string]: string }) {
	...
	private async _createLogsInGraph() {
		while (!this._spinalQueue.isEmpty()) {
			...
			const actualState = log.type.toLowerCase() === "alert" ? WEBSOCKET_STATE.alert : WEBSOCKET_STATE.running;
	...
	private async _changeBuildingState(actualState: WEBSOCKET_STATE): Promise<void> {
		const spinalLog = await this.getLogModel();
		if (!spinalLog) return;

		return SpinalServiceLog.getInstance().changeWebsocketState(spinalLog, actualState);
	}
	...
	private _fileExistInDirectory(directory: spinal.Directory, fileName: string): spinal.File | undefined {
